...
Future<void> exportUltrasoundPDF(
  ...
  try {
    ...
    final iconSvg = await rootBundle
        .loadString('assets/images/modality_icons/ultrasound.svg');
  ...
  // single bad/slow URL can't stall the whole export.
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
        // =========================== COVER PAGE ===========================
        _buildCoverPage(logoWidget, heroImage, docRef, modalityIcon, 'Ultrasound'),
        pw.NewPage(),
  ...
  // Prefer the user-entered document name (SysGenDocName) if one was set;
  // fall back to Brand_Model like the angiography exporter.
  final rawName = docRef.sysGenDocName.trim().isNotEmpty
      ? docRef.sysGenDocName.trim()
      : '${docRef.sysGenBrand}_${docRef.sysGenModal}';
  final fileName = '${rawName}_Report.pdf'.replaceAll(' ', '_');
...
// ---------------------------------------------------------------------------
// INFO TABLE — zebra-striped label/value rows, blank values render as "N/A"
...
// ---------------------------------------------------------------------------
// PROBES TABLE — Probe1..Probe5 (Model / YOM / Condition), skips rows where
// all three values are empty (i.e. that probe slot wasn't used).
// ---------------------------------------------------------------------------
pw.Widget _probesTable(UltrasoundRecord docRef) {
  final entries = <List<String>>[
    ['1', docRef.probe1Model, docRef.probe1YOM, docRef.probe1Condition],
    ['2', docRef.probe2Model, docRef.probe2YOM, docRef.probe2Condition],
    ['3', docRef.probe3Model, docRef.probe3YOM, docRef.probe3Condition],
    ['4', docRef.probe4Model, docRef.probe4YOM, docRef.probe4Condition],
    ['5', docRef.probe5Model, docRef.probe5YOM, docRef.probe5Condition],
  ]
      .where(
  ...
  if (entries.isEmpty) {
    return pw.Text('No probes recorded.',
  ...
  return pw.Table(
    ...
    children: [
      pw.TableRow(children: [
        headerCell('#'),
        headerCell('Model'),
        headerCell('Year of Manufacture'),
