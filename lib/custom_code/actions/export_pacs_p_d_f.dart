...
Future<void> exportPacsPDF(
  ...
  try {
    ...
    final iconSvg = await rootBundle
        .loadString('assets/images/modality_icons/pacs.svg');
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
        // =========================== COVER PAGE ===========================
        _buildCoverPage(logoWidget, heroImage, docRef, modalityIcon, 'PACS'),
        pw.NewPage(),
...
// ---------------------------------------------------------------------------
pw.Widget _cassettesTable(PacsRecord docRef) {
